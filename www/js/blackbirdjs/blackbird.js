			...
			'<div class="header">',
				...
				'<div class="right">',
					'<div id="', IDs.controls, '" class="controls">',
						...
						'<span class="btnClear" title="clear" op="clear"></span>',
						'<span class="btnClose" title="close" op="close"></span>',
					'</div>',
				'</div>',
			'</div>',
			'<div class="main">',
		...
		debug:
			function() {
				for(var i = 0,arg; arg = arguments[i]; i++) {
					// firebug存在是优先使用console
					if (typeof(console) != 'undefined' && console.debug)
						console.debug(arg);
					else
						addMessage( 'debug', arg );
				}
			},
		warn:
		...
		profile:
			function( label ) {
				...
				if ( label == undefined || label == '' ) {
					var msg = 'Please specify a label for your profile statement';
					if (typeof(console) != 'undefined' && console.error)
						console.error(msg);
					else
						addMessage( 'error', '<b>ERROR:</b> ' + msg );
				}
				else if ( profiler[ label ] ) {
					var msg =  [ label, ': ', currentTime - profiler[ label ],	'ms' ].join( '' );
