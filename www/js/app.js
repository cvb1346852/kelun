...
$(document).ready(function() {
	...
	var onPostMessage = function (e) {
		var params = {};
		try {
			params = JSON.parse(e.data.toString());
		} catch (e) {
		}
		$.each(params, function (index, item) {
			if (index == 'storageClear') {
				localStorage.clear();
				location.reload();
			}
			if (index == 'scroll') {
				$ips.scrollHeight = item;
				scrollSmallBox(item);
			}
			if (index == 'userTheme') {
				defaults_style(item);
...
function loadScript(scriptName, callback, reload, islocalfile) {
	...
	if (!jsArray[scriptName] || reload == true) {
		jsArray[scriptName] = true;
	        ...
	        if(privateUrl.test(strName) || islocalfile === true) {
	            ...
	            script.src = RESOURCEURL+scriptName;
	        }
...
function checkURL() {
	...
	if (url) {
		...
		sendMessageToGateway({nohash:true});
...
function loadURL(url, container) {
	...
	if (window.top != window && /\_\_displayrunner\_\_=gateway/.test(url) == false) {
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (window.top != window) {
				...
				$("#wid-id-0 .jarviswidget-fullscreen-btn").bind("click",function(){
					sendMessageToGateway({'fullscreen' : 1});
				});
			}
		},
...
function drawBreadCrumb() {
	...
	$("#ribbon ol.breadcrumb").append($("<li>首页</li>"));
...
//弹出信息框跟随垂直滚动
function scrollSmallBox(height){
	var sHeight = height - 100;
	if (height < 100) {
		sHeight = 0;
	}
	var ssHeight = sHeight + 50;
	$("#divSmallBoxes").css({"top" : sHeight});
	$(".modal-dialog").css({"margin-top" : ssHeight});
	$('#Msg1').css({
		top : height + 60
	});
}
