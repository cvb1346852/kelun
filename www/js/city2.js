...
function BindProvince(){
    ...
    var toloProvince = document.getElementById("toloProvince2");
    toloProvince.innerHTML = "";
    toloProvince.options[0] = new Option(opt0,"");
    for(var i=0; i<ProvinceCount; i++){
        toloProvince.options[i+1] = new Option(china[i][0],china[i][0]);
    }
}

function BindCity2(City){
    var opt0 = "请选择";
    var ProvinceCount=china.length;
    var toloProvince = document.getElementById("toloProvince2");
    toloProvince.innerHTML = "";
    ...
    var opt0City = "请选择";
    var toloCity = document.getElementById("toloCity2");
    toloCity.innerHTML = "";
    toloCity.options[0] = new Option(opt0City,"");
    ...
    for(var i=0; i<ProvinceCount; i++){
        ...
        toloProvince.options[i+1] = new Option(china[i][0],china[i][0]);
        if(flag && chose){
            toloProvince.options[i+1].selected = true;
            chose=false;
        }
    }
    ...
    for(var i=0; i<cityCount; i++){
        if(cityCount == 1 && i == 0){
            toloCity.options[i+1] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
            i = 1;
        }
        else if(cityCount > 1 && i == 0){
            i = 1;
            toloCity.options[i] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
        }
        else{
            toloCity.options[i] = new Option(china[selectProvinceIndex][i],china[selectProvinceIndex][i]);
        }
        if(china[selectProvinceIndex][i]==City){
            toloCity.options[i].selected=true;
        }
    }

    if($("#ddlProvince option:selected").val() === ""){
        $('#toloCity2').html('<option value="" selected>请选择</option>')
    }

}

function selectMoreCity2(sbj){
    var opt0 = "请选择";
    if(sbj.selectedIndex==0){
        var toloCity = document.getElementById("toloCity2");
        toloCity.innerHTML = "";
        toloCity.options[0] = new Option(opt0,"");
        return;
    }
    ...
    for(var i=0; i<ProvinceCount; i++){
        if(china[i][0] == selectProvince){
            ...
            for(var j=0; j<cityCount; j++){
                if(cityCount == 1 && j == 0){
                    toloCity.options[j+1] = new Option(china[i][j],china[i][j]);
                    j = 1;
                }
                else if(cityCount > 1 && j == 0){
                    j = 1;
                    toloCity.options[j] = new Option(china[i][j],china[i][j]);
                }
                else{
                    toloCity.options[j] = new Option(china[i][j],china[i][j]);
                }
                if(j == 1){
                    toloCity.options[1].selected = true;
                }
            }
            break;
        }
