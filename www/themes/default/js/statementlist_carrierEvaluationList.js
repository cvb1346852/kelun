...
loadScript("/js/hui/jquery.hui.grid.js",function(){
    $("#tblMain").grid({
        "aoColumns":[
            ...
            {sTitle: "运输服务质量", sName: "total_grade","bSortable": true},
			{sTitle: "竞标回标率", sName: "quote_rate","bSortable": true},
            {sTitle: "到达时效", sName: "arraive_num","bSortable": true},
            {sTitle: "派车时效", sName: "send_car","bSortable": true},
            {sTitle: "运输质量", sName: "order_num","bSortable": true},
            {sTitle: "中标违约率", sName: "","bSortable": false},
            {sTitle: "提供位置服务率", sName: "lbs_rate","bSortable": true}

    ],
        "fnServerData":function(sSource,aoData,fnCallback){
            var searchParams = $("#frmSearch").serializeArray();
            $ips.gridLoadData(sSource,aoData,fnCallback,"statementlist","getCarrierEvaluationList",searchParams,function(data){
                if(data.result){
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"承运商考核报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'getCarrierEvaluationList',searchParams: exportSearchParams,title:'承运商考核报表',partDataRows:3000,partSize:100});
},true,true);
