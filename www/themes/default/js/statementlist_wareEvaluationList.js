...
loadScript("/js/hui/jquery.hui.grid.js",function(){
    $("#tblMain").grid({
        "aoColumns":[
            ...
            {sTitle: "装货质量", sName: "load_quality","bSortable": true},
            {sTitle: "基地服务质量", sName: "total_grade","bSortable": true}

    ],
        "fnServerData":function(sSource,aoData,fnCallback){
            var searchParams = $("#frmSearch").serializeArray();
            $ips.gridLoadData(sSource,aoData,fnCallback,"statementlist","getWareEvaluationList",searchParams,function(data){
                if(data.result){
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"基地考核报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'getWareEvaluationList',searchParams: exportSearchParams,title:'基地考核报表',partDataRows:3000,partSize:100});
},true,true);
