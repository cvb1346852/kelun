...
function set_time_range(){
    var returntimes = moment().startOf('week').format('YYYY-MM-DD HH:mm')+' - '+ moment().endOf('week').format('YYYY-MM-DD HH:mm');
    return returntimes;
}
/*加载出发地*/
$(function() {
...
loadScript('js/hui/jquery.hui.grid.js', function () {
    $('#shipmentTbl').grid({
        "aoColumns" : [
            ...
            {sTitle: "开标时间差", sName: "kaibiao","bSortable": false},
            {sTitle: "评标时间差", sName: "pingbiao","bSortable": false},
        ],
        "fnServerData" : function(sSource, aoData, fnCallback) {
            var searchParams = genSearchParams();
            $ips.gridLoadData(sSource, aoData, fnCallback, "statementlist", "tenderlist", searchParams, function(data) {
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"发标,中标时间明细报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'tenderlist',searchParams: exportSearchParams,title:'发标,中标时间明细报表',partDataRows:3000,partSize:100});
},true,true);
