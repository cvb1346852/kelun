...
loadScript("/js/hui/jquery.hui.grid.js",function(){
    $("#tblMain").grid({
        "aoColumns":[
            ...
            {sTitle: "联系人", sName: "relation_person","bSortable": false},
            {sTitle: "手机号", sName: "relation_phone","bSortable": false},
            {sTitle: "运输次数", sName: "delivery_times","bSortable": true},
            {sTitle: "运输质量", sName: "order_num","bSortable": true},
			{sTitle: "运单金额", sName: "total_price","bSortable": true},
            {sTitle: "订单评分", sName: "total_grade","bSortable": true},
            {sTitle: "投诉量", sName: "complain_num","bSortable": true},
            {sTitle: "运输距离", sName: "total_distance","bSortable": true}

    ],
        "fnServerData":function(sSource,aoData,fnCallback){
            var searchParams = $("#frmSearch").serializeArray();
            $ips.gridLoadData(sSource,aoData,fnCallback,"statementlist","getCarrierRankingList",searchParams,function(data){
                if(data.result){
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"承运商排名报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'getCarrierRankingList',searchParams: exportSearchParams,title:'承运商排名报表',partDataRows:3000,partSize:100});
},true,true);
