...
loadScript("/js/hui/jquery.hui.grid.js",function(){
    $("#tblMain").grid({
        "aoColumns":[
            ...
            {sTitle: "竞标时间", sName: "create_time","bSortable": false},
            ...
            {sTitle: "是否中标", sName: "status","bSortable": false},
            {sTitle: "中标价格", sName: "price","bSortable": false}

    ],
        "fnServerData":function(sSource,aoData,fnCallback){
            var searchParams = $("#frmSearch").serializeArray();
            $ips.gridLoadData(sSource,aoData,fnCallback,"statementlist","getCarrierBidDetail",searchParams,function(data){
                if(data.result){
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"carrierQueteList-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'getCarrierBidDetail',searchParams: exportSearchParams,title:'承运商竞标明细报表',partDataRows:3000,partSize:100});
},true,true);
