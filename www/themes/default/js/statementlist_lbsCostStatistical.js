...
loadScript('js/hui/jquery.hui.grid.js', function () {
    $('#shipmentTbl').grid({
        "aoColumns" : [
            {sTitle: "单位", sName: "company","bSortable": false},
            {sTitle: "单位类型", sName: "company_type","bSortable": true},
            {sTitle: "条数", sName: "total","bSortable": false},
            {sTitle: "金额", sName: "total_price","bSortable": false},
        ],
        "fnServerData" : function(sSource, aoData, fnCallback) {
            var searchParams = genSearchParams();
            $ips.gridLoadData(sSource, aoData, fnCallback, "statementlist", "lbsConstStatistical", searchParams, function(data) {
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"lbs费用统计报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'lbsConstStatistical',searchParams: exportSearchParams,title:'lbs费用统计报表',partDataRows:3000,partSize:100});
},true,true);
