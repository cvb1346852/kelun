...
loadScript("/js/hui/jquery.hui.grid.js",function(){
    $("#tblMain").grid({
        "aoColumns":[
			...
			{sTitle: "被取消中标方", sName: "relation_name","bSortable": false},
            {sTitle: "制单时间", sName: "shipment_time","bSortable": false},
            ...
            {sTitle: "价格", sName: "quote_price","bSortable": false},
            {sTitle: "发标时间", sName: "tender_time","bSortable": false},
            {sTitle: "取消中标时间", sName: "create_time","bSortable": false},
            {sTitle: "责任方", sName: "responsible_party","bSortable": false},
            {sTitle: "取消中标备注", sName: "remark","bSortable": false}

    ],
        "fnServerData":function(sSource,aoData,fnCallback){
            var searchParams = $("#frmSearch").serializeArray();
            $ips.gridLoadData(sSource,aoData,fnCallback,"statementlist","getAbandWinBidList",searchParams,function(data){
                if(data.result){
...
loadScript('js/hui/jquery.hui.exportdata.js', function () {
    ...
    exportSearchParams.push({name:"filename","value":"取消中标报表-" + time});
    $('#export').exportdata({dataModule : 'statementlist',dataMethod:'getAbandWinBidList',searchParams: exportSearchParams,title:'取消中标报表',partDataRows:3000,partSize:100});
},true,true);
