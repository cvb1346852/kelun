...
$(function() {	
    ...
    $("#editPage").click(function (){
        $ips.locatesubsystem("project/motorcade/edit.html",false);
    });
...
function runDataTables() {
	$('#tblMain').grid({
		...
		"fnServerData" : function(sSource, aoData, fnCallback) {
			$ips.gridLoadData(sSource, aoData, fnCallback, "motorcade", "search", searchParams, function(pager) {
...
//获取选中的id
function edit(id) {
    $ips.locate("motorcade", "edit", "id="+id);
}
...
function del() {
    ...
    $ips.confirm("您确定要删除这"+ids.length+"条记录吗?",function(btn) {
        if (btn == "确定") {
            $ips.lockPage();
            $ips.load("motorcade", "del", "id=" + id, function(result){
                $ips.unLockPage();
                if(result) {
                    $ips.succeed("操作成功！");
