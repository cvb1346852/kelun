...
<style lang="less">
    ...
    #current-point{ width: 6.25rem; position: fixed; left: .75rem; top: .75rem; background-color: rgba(0,0,0,.9); border-radius: .25rem; overflow: hidden}
    ...
    .trail-icon{
        width: 15px;
        height: 15px;
        background: url(../../../static/lib/img/map_ctrl_icon.png) no-repeat -60px -5px;
...
<script>
    ...
    const formatTime = Vue.filter('date', filters.dateFilter)

    var toTimestamp = Date.parse(new Date())
    toTimestamp /= 1000 // 当前时间时间戳,秒为单位
    var from1dayTimestamp = toTimestamp - ( 60 * 60 * 24 ) // 一天前时间戳,秒为单位
    var from3dayTimestamp = toTimestamp - ( 60 * 60 * 24 * 3 ) // 一天前时间戳,秒为单位
    var from7dayTimestamp = toTimestamp - ( 60 * 60 * 24 * 7 ) // 一天前时间戳,秒为单位
    var globe_Marker = [];
    var marker;
    ...
    export default {
        data() {
            return {
                ...
                currentIndex: 1,
                from1dayTime: formatTime(from1dayTimestamp, 'MM-dd hh:mm'),
                from3dayTime: formatTime(from3dayTimestamp, 'MM-dd hh:mm'),
                from7dayTime: formatTime(from7dayTimestamp, 'MM-dd hh:mm'),
                toTime: formatTime(toTimestamp, 'MM-dd hh:mm'),
                fromTime: formatTime(from1dayTimestamp, 'MM-dd hh:mm'),

                from1dayTimeDetail: formatTime(from1dayTimestamp, 'yyyy-MM-dd hh:mm:ss'),
                from3dayTimeDetail: formatTime(from3dayTimestamp, 'yyyy-MM-dd hh:mm:ss'),
                from7dayTimeDetail: formatTime(from7dayTimestamp, 'yyyy-MM-dd hh:mm:ss'),
                toTimeDetail: formatTime(toTimestamp, 'yyyy-MM-dd hh:mm:ss'),
                fromTimeDetail: formatTime(from1dayTimestamp, 'yyyy-MM-dd hh:mm:ss'),
                isShowInfo: true,
                ...
                ptsLen: 0,
                curPoint: {
                    time: formatTime(from1dayTimestamp, 'hh:mm:ss/MM-dd'),
                    speed: 0
                },
                sliderInfo: {
                    timer: null,//定时器
...
//                    omap.setZoom(18)
            ...
            fn_addTruck(lng, lat) {
                var markeropt = {
                    icon : {
                        url : 'static/lib/img/truck_stop_slected.png',
                        size : [28,28],
                        imageSize: [28,28],
                        anchor : [14,14]
                    },
            ...
            fn_addMarkerUser(pt, type) {
                ...
                if ( 'first' === type ) {
                    imgUrl = 'static/lib/img/truck_start.png'
                }
                else if ( 'last' === type ) {
                    imgUrl = 'static/lib/img/truck_end.png'
                }
