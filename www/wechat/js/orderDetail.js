...
$.sendData('shipment','getOrderSourceDetail',urlParam,'shipment',function(data){
	$('#tblMain').html('');
	if(data[0].tenderMsg){
		$('#tender_id').val(data[0].tenderMsg.id);
		$('#cooperate_type').val(data[0].tenderMsg.cooperate_type);
		$('#ctime').text(data[0].tenderMsg.tender_limit);
	}
	if(data[0].status==2){
		$('#cd').show();	
	}
	$('#phone').html(data[0].ware.person+'&nbsp;&nbsp;'+data[0].ware.phone);
	$.each(data, function(i, item) {
		var html='';
		html+='<tr>';
		html+='<td style="width:25%">'+item.from_address+'</td>';
		html+='<td style="width:25%">'+item.to_address+'</td>';
		html+='<td style="width:25%">'+Math.floor(item.quality)+'</td>';
		html+='<td style="width:25%">'+Math.floor(item.weight)+'</td>';
		html+='</tr>';
		$('#tblMain').append(html);
	});
});
...
$.sendData('shipment','getOrderSourceDetail',urlParam,'shipment',function(data){
	if(data) {
		...
		var html ='';
		html+='<div class="weui_cells_tips null-notice">暂无相关信息</div>';
		$('.container').html(html);
	}
});
